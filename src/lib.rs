        ...
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                VoicevoxError::SynthesisError {
                    ...
                    write!(
                        f,
                        "Synthesis failed for style_id {} (text length: {}): {}",
                        style_id,
                        text.len(),
                        source
                    )
                }
                VoicevoxError::ModelError {
                    model_path,
                    operation,
                    source,
                } => {
                ...
                VoicevoxError::VoiceResolutionError {
                    input,
                    available_options,
                    source,
                } => {
                    write!(
                        f,
                        "Voice resolution failed for '{}' (available: {}): {}",
                        input,
                        available_options.join(", "),
                        source
                    )
                }
                VoicevoxError::ConfigError {
                    field,
                    value,
                    constraint,
                } => {
                    write!(
                        f,
                        "Configuration error: field '{}' value '{}' violates constraint '{}'",
